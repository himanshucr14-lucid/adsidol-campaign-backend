...
async function logEvent(userId, event, isAgency = false) {
    ...
    const key = isAgency ? `adsidol:agency_analytics:${userId}` : `adsidol:analytics:${userId}`;
    await redis('RPUSH', key, JSON.stringify(event));
}

async function getAnalytics(userId, isAgency = false) {
    const key = isAgency ? `adsidol:agency_analytics:${userId}` : `adsidol:analytics:${userId}`;
...
// ── Agency Store (contacts + template + followups) ─────────────────────────
async function getAgency(userId) {
    const key = `adsidol:agency:${userId}`;
    const raw = await redis('GET', key);
    return raw ? JSON.parse(raw) : null;
}

async function setAgency(userId, payload) {
    const key = `adsidol:agency:${userId}`;
    await redis('SET', key, JSON.stringify(payload));
}

module.exports = { saveJob, getJob, updateJob, deleteJob, getAllJobs, logEvent, getAnalytics, getTemplates, setTemplates, getContacts, setContacts, getAgency, setAgency };
