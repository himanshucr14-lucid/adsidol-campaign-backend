// ═══════════════════════════════════════════════
        ...
        let appMode = 'client'; // 'client' or 'agency'

        function setAppMode(mode, skipSync = false) {
            appMode = mode;
            document.body.setAttribute('data-mode', mode);
            
            // Update Toggle UI
            const modeClientBtn = document.getElementById('modeClientBtn');
            const modeAgencyBtn = document.getElementById('modeAgencyBtn');
            if (modeClientBtn && modeAgencyBtn) {
                modeClientBtn.classList.toggle('active', mode === 'client');
                modeAgencyBtn.classList.toggle('active', mode === 'agency');
            }

            // Sync Header Title
            const titleEl = document.getElementById('topNavTitle');
            if (titleEl) {
                titleEl.textContent = mode === 'agency' ? 'Agency Dashboard' : 'Campaign Dashboard';
            }
        ...
        function toggleAppMode() {
            setAppMode(appMode === 'client' ? 'agency' : 'client');
        }

        // Initialize mode from localStorage
        (function initMode() {
            const saved = localStorage.getItem('adsidol_app_mode');
            if (saved === 'agency') {
                setTimeout(() => setAppMode('agency', true), 100);
            }
        })();
        ...
        async function saveAgencyDataToCloud() {
            if (!currentApiKey || appMode !== 'agency') return;
            try {
                const res = await fetch(`${BACKEND_URL}/api/contacts?type=agency`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'x-api-key': currentApiKey },
                    body: JSON.stringify({ 
                        agencyContacts, 
                ...
                const data = await res.json();
                if (!data.ok) console.warn('Cloud agency sync failed:', data.error);
            } catch (e) { console.error('Cloud agency sync error:', e); }
        }

        async function loadAgencyDataFromCloud() {
            if (!currentApiKey) return;
            try {
                const res = await fetch(`${BACKEND_URL}/api/contacts?type=agency`, {
                    headers: { 'x-api-key': currentApiKey }
                ...
                const data = await res.json();
                if (data.ok && data.data) {
                    const { agencyContacts: contactsArr, agencyTemplate: tpl, agencyFollowups: fus } = data.data;
                    if (contactsArr) agencyContacts = contactsArr;
        ...
        // Agency Mode State Silo
        let agencyContacts = [];
        let agencyTemplate = { 
            subject: 'Scaling {{agency_name}} with Adsidol?', 
            body: 'Hi {{contact_name}},\n\nSaw {{agency_name}} is doing great work in {{vertical}}.\n\nWe provide high-quality leads for agencies with 40% higher conversion rates.\n\nInterested in a quick chat?\n\nBest,\nYour Name\nAdsidol' 
        };
        let agencyFollowups = [
            { step: 1, delayDays: 3, subject: 'Re: Scaling {{agency_name}}', body: 'Hi {{contact_name}},\n\nJust following up on my previous note...' },
            { step: 2, delayDays: 7, subject: 'Quick question for {{agency_name}}', body: 'Hi {{contact_name}},\n\nWanted to check if you had a moment...' },
            { step: 3, delayDays: 14, subject: 'Adsidol + {{agency_name}}', body: 'Hi {{contact_name}},\n\nThird time is a charm...' },
            { step: 4, delayDays: 21, subject: 'Final follow up for {{agency_name}}', body: 'Hi {{contact_name}},\n\nLast try...' }
        ];
        ...
        function matchVertical(subjectVertical) {
            ...
            // 2. Partial match (subject is generic inside full string e.g. "ecommerce" in "Ecommerce (AOV)")
            found = VERTICALS.find(v => v.toLowerCase().includes(lowerRaw));
            if (found) return found;

            // 3. Partial match backward
            found = VERTICALS.find(v => lowerRaw.includes(v.toLowerCase()));
            if (found) return found;
