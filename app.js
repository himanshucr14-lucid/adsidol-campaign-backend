// ═══════════════════════════════════════════════
        ...
        // ═════════════════════════════════════════════════════════════
        // ═════════════════════════════════════════════════════════════
        // EDGE NAV ENGINE v5 — Professional "Pop-up at edge" buttons
        // ═════════════════════════════════════════════════════════════
        ...
        if (mainContentEl) {
            ...
            function buildEdgeNav() {
                if (topBtn) return;
                
                topBtn = document.createElement('button');
                topBtn.className = 'edge-nav-btn edge-nav-top';
                topBtn.innerHTML = `<span class="os-icon">↑</span><span class="os-label">Previous</span>`;
                document.body.appendChild(topBtn);

                botBtn = document.createElement('button');
                botBtn.className = 'edge-nav-btn edge-nav-bot';
                botBtn.innerHTML = `<span class="os-label">Next</span><span class="os-icon">↓</span>`;
                document.body.appendChild(botBtn);

                topBtn.addEventListener('click', () => doJump(-1));
                botBtn.addEventListener('click', () => doJump(1));
            }
            ...
            function doJump(dir) {
                ...
                if (botBtn) botBtn.classList.remove('visible');
            }
            ...
            function updateEdgeNav() {
                ...
                const atBot = scrollable ? (sh - st - ch) <= 4 : true; 

                const prevLabel = getTarget(-1);
                const nextLabel = getTarget(1);

                if (atTop && prevLabel) {
                    topBtn.querySelector('.os-label').textContent = 'Go to ' + prevLabel;
                ...
                if (atBot && nextLabel) {
                    botBtn.querySelector('.os-label').textContent = 'Go to ' + nextLabel;
                    botBtn.classList.add('visible');
                } else {
                    botBtn.classList.remove('visible');
                }
            }

            mainContentEl.addEventListener('scroll', () => {
                if (!edgeRafId) edgeRafId = requestAnimationFrame(updateEdgeNav);
            }, { passive: true });
            
            // Initial check after a delay to let layout settle
            setTimeout(updateEdgeNav, 500);
            window._osMarkUnsettled = updateEdgeNav; // Overwrite to just force an update
            ...
            observer.observe(mainContentEl, { childList: true, subtree: true });
        }
        // ═══════════════════════════════════════════════
        // STATE
        // ═══════════════════════════════════════════════
